...
class ConnectionEntry implements ConnectionContext {
    ...
    protected ConnectionEntry(final long connuid, final Connection conn, final ConnectionConfiguration config,
    ...
    public <R, V> R executeCommand(final long objectUID, final Command<R, V> cmd, final CallingContext ctx)
            throws SQLException {
        try {
            lock.lock();
            active = true;
            ...
            if (target != null) {
                ...
                if (rr instanceof JdbcObjectTransport) {
                    ...
                    jdbcObjects.put(ruid,
                            new JdbcObjectHolder<V>((V) transport.getJDBCObject(), ctx, jdbcInterfaceType));
                    if (LOGGER.isDebugEnabled()) {
    ...
    private static <T> JdbcObjectTransport<T> checkResult(final Object obj, final AtomicLong counter) {
        if (obj instanceof StatementLocalJdbcObjectTransport) {
            ...
            final JdbcObjectTransport<T> retval = new StatementRemoteJdbcObjectTransport(uid, t.getJDBCObject(),
                    t.getMaxRows(), t.getQueryTimeout());
            return retval;
            ...
            final StatementRemoteJdbcObjectTransport<Statement> retval = StatementRemoteJdbcObjectTransport.of(uid,
                    stmt, maxRows, queryTimeOut);
    ...
    public void cancelCurrentStatementExecution(final long itemuid, final StatementCancelCommand cmd)
            throws SQLException {
    ...
    private StreamingResultSet handleResultSet(final ResultSet result, final boolean forwardOnly,
            final CallingContext ctx) throws SQLException {
        ...
        jdbcObjects.put(id, new JdbcObjectHolder<ResultSetHolder>(new ResultSetHolder(result, executor, rowPacketSize,
                lastPartReached), ctx,
