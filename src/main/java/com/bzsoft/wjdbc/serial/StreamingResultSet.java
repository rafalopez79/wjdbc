...
public class StreamingResultSet implements ResultSet, Externalizable {
    ...
    public StreamingResultSet(final int rowPacketSize, final boolean forwardOnly, final boolean prefetchMetaData,
    ...
    public void readExternal(final ObjectInput in) throws IOException, ClassNotFoundException {
        ...
        for (final String cName : columnNames) {
            columnNameMap.put(cName, i);
            i++;
        }
        i = 1;
        for (final String cName : columnLabels) {
            columnLabelMap.put(cName, i);
            i++;
        }
    ...
    public boolean populate(final ResultSet rs) throws SQLException {
        ...
        if (prefetchMetaData) {
            metaData = new SerialResultSetMetaData(md);
        }
    ...
    public InputStream getBinaryStream(final int columnIndex) throws SQLException {
        ...
        if (preGetCheckNull(ci)) {
            ...
            if (obj instanceof byte[]) {
                ...
                final String msg = "StreamingResultSet.getBinaryStream(): Can't convert object of type '"
                        + obj.getClass() + "' to InputStream";
    ...
    private static BigDecimal internalGetBigDecimal(final Object value, final int columnType, final int scale)
            throws SQLException {
    ...
    private int getIndexForName(final String name) throws SQLException {
        ...
        Integer pos = columnNameMap.get(nameLowercase);
        if (pos == null) {
            pos = columnLabelMap.get(nameLowercase);
        }
        result = pos != null ? pos.intValue() : -1;
        if (result < 0) {
    ...
    public RowId getRowId(final int parameterIndex) throws SQLException {
        return commandSink.process(remainingResultSet,
                ReflectiveCommand.<RowId, Object> of(JdbcInterfaceType.RESULTSETHOLDER, "getRowId",
