...
public final class StreamingResultSet implements ResultSet, Externalizable {
	...
	public boolean populate(final ResultSet rs) throws SQLException {
		...
		for (int i = 1; i <= columnCount; i++) {
			final int j = i - 1;
			columnTypes[j] = md.getColumnType(i);
			columnNames[j] = md.getColumnName(i).toLowerCase();
			columnLabels[j] = md.getColumnLabel(i).toLowerCase();
		}
		int i = 1;
		for (final String cName : columnNames) {
			columnNameMap.put(cName, i);
	...
	private static final BigDecimal internalGetBigDecimal(final Object value, final int columnType, final int scale) throws SQLException {
