...
public class RowPacket implements Externalizable {
        ...
        if (rowCount > 0) {
            ...
            if (rowCount != maxrows) {
                for (final FlattenedColumnValues fcv : flattenedColumnsValues) {
                    FlattenedColumnValues.compress(fcv, rowCount).writeExternal(out);
                }
            } else {
                for (final FlattenedColumnValues fcv : flattenedColumnsValues) {
        ...
        while (rs.next()) {
            for (int i = 1; i <= columnCount; i++) {
                ...
                case Types.ARRAY:
                    ...
                    break;
                case Types.STRUCT:
                    flattenedColumnsValues[internalIndex].setObject(rowCount,
                            new SerialStruct((Struct) rs.getObject(i)));
                    break;
                case ORACLE_ROWID:
    ...
    private void prepareFlattenedColumns(final ResultSetMetaData metaData, final int columnCount) throws SQLException {
        ...
        for (int i = 1; i <= columnCount; i++) {
            ...
            flattenedColumnsValues[i - 1] = new FlattenedColumnValues(componentType, maxrows == 0 ? DEFAULT_ARRAY_SIZE
                    : maxrows);
        }
    }
