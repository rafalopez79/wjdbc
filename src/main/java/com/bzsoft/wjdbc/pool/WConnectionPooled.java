...
public class WConnectionPooled implements Connection {

	private final PooledConnectionSubject	subject;
	private final Connection					connection;

	public WConnectionPooled(final Connection connection, final PooledConnectionSubject subject) {
	...
	public Statement createStatement() throws SQLException {
		try {
			...
			final Statement wrapper = new WStatementPooled(stmt, subject);
			return wrapper;
		} catch (final SQLException e) {
			final SQLException sqle = e;
			subject.connectionErrorOccurred(sqle);
			throw sqle;
		} catch (final Throwable th) {
			final SQLException sqle = new SQLException(th);
			subject.connectionErrorOccurred(sqle);
			throw sqle;
		}
	}
	...
	public PreparedStatement prepareStatement(final String sql) throws SQLException {
		try {
			...
			final PreparedStatement wrapper = new WPreparedStatementPooled(pstmt, subject);
			subject.addStatement(pstmt);
			return wrapper;
		} catch (final SQLException e) {
			subject.connectionErrorOccurred(e);
			throw e;
		} catch (final Throwable th) {
			final SQLException sqle = new SQLException(th);
			subject.connectionErrorOccurred(sqle);
	...
	public CallableStatement prepareCall(final String sql) throws SQLException {
		try {
			...
			final CallableStatement wrapper = new WCallableStatementPooled(cstmt, subject);
	...
	@Override
	public void setSchema(final String sc) throws SQLException {
		SQLException sqle = null;
		try {
			connection.setSchema(sc);
		} catch (final SQLException e) {
			sqle = e;
			throw sqle;
		} catch (final Throwable th) {
